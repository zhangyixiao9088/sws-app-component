	...
	try {
		...
		if(serverData){
			if(serverData.indexOf('t:')>=0){
				...
				addSuccessFileInfo(this,tableId,file.id,"上传成功",info);	
			}else if(serverData.indexOf('f:')>=0){
				var info = serverData.substr(2);
				addFileInfo(tableId,file.id,"上传失败",'');	
			}
		}
...
function addSuccessFileInfo(swfu,tableId,fileId,message,pars){
	...
	row.cells[0].innerHTML = "<a href='"+url+"'>"+cel1Txt+"</a>";
...
function fileQueued(file){
	...
	addReadyFileInfo(file,vId,"待上传","");
}


function addServerFile(service,tableId,fileId,fileName,fileSize,msg,isEdit){
	...
	row.cells[1].innerHTML= fileSize;
	if(isEdit){
	...
	$.ajax({
           ...
           success: function(serverData){
           		$(serverData).find("ds>r").each(function(){ 
            		...
            		addServerFile(service,tableId,fileId,fileName,fileSize,'已上传',isEdit);
            	});
	    	}
	});
}
