function onckFromDate(){
	...
	if(v=="Y"){
		...
		WdatePicker({el:'dateFrom',dateFmt:'yyyyMMddHH:mm',disabledDates:['^19','^200']}); 
	}else if(v=="M"){
		WdatePicker({el:'dateFrom',dateFmt:'yyyyMM',disabledDates:['^19','^200']}); 
	}else if(v=="D"){
		WdatePicker({el:'dateFrom',dateFmt:'yyyyMMdd',disabledDates:['^19','^200']}); 
	} else if(v=="H"){
		WdatePicker({el:'dateFrom',dateFmt:'yyyyMMddHH',disabledDates:['^19','^200']}); 
	} 
}
function onckToDate(){
	var dc=document.getElementById("dateType");
	...
	if(v=="Y"){
		...
		WdatePicker({el:'dateFrom',dateFmt:'yyyyMMddHHmm',disabledDates:['^19','^200']}); 
	}else if(v=="M"){
		WdatePicker({el:'dateTo',dateFmt:'yyyyMM',disabledDates:['^19','^200']}); 
	}else if(v=="D"){
		WdatePicker({el:'dateTo',dateFmt:'yyyyMMdd',disabledDates:['^19','^200']}); 
	} else if(v=="H"){
		WdatePicker({el:'dateTo',dateFmt:'yyyyMMddHH',disabledDates:['^19','^200']}); 
	} 
}
function initDate(){
	 var dc=document.getElementById("dateType");
	 ...
	 if(type=='Y'){
	 	...
	 	if(dateFrom){
	 		dateFrom.value= ((year-1).toString()+"-"+month.toString());
	 	}
	 	...
	 	if(dateFrom){
	 		dateFrom.value= (year.toString()+"-"+month.toString()+"-"+day.toString()+" "+(hour-1).toString()+":"+minutes); ;
	 	}
	 	dateTo.value=(year.toString()+"-"+month.toString()+"-"+day.toString()+" "+hour.toString()+":"+minutes); ;
	 }	
}
...
function onckDate(){
	...
	if(v=="Y"){
		...
		WdatePicker({el:'dateIpt',dateFmt:'yyyyMM',disabledDates:['^19','^200']}); 
	}else if(v=="D"){
		WdatePicker({el:'dateIpt',dateFmt:'yyyyMMdd',disabledDates:['^19','^200']}); 
	} else if(v=="H"){
		WdatePicker({el:'dateIpt',dateFmt:'yyyyMMddHH',disabledDates:['^19','^200']}); 
	} 
}
function getDate(){
	var dc=document.getElementById("dateSel");
